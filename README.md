# チーム名

VeRyHacker

# プロダクト名

プロダクト名を記載してください。

## 概要
MetaQuest3sとUnityのハンドトラッキングを利用して手の形を認識することで、
VRのカメラから読み取った手のデータを文字に変換して手話を翻訳が可能になることによる
耳の不自由な方とのコミュニケーションがとれるようにする。
何を作ったのかを3〜5行程度で説明してください。

## デモ

以下を掲載してください。

- 発表資料URL（必須）
- デモURL（任意）
- デモ動画（任意）
- スクリーンショット（1枚以上推奨）

## システム構成

Unity6(version : 6000.0.73f1)
Blender()

アプリケーション全体の構成が分かる図や説明を記載してください。

## 背景・課題

最近、目が不自由な人間とその周りのヒューマンドラマが描かれたアニメが放送されており、
それを見たときにどうにかしてこのような人々とコミュニケーションをとることができないか
考えた。そのとき、
なぜこのプロダクトを作ったのか、どのような課題を解決したいのかを記載してください。

## 主な機能

- 機能1
- 機能2
- 機能3

## 工夫した点・こだわった点

技術面・UI/UX・アイデア面など、特に工夫した点やチャレンジした点を記載してください。

## 使用技術

- フロントエンド：
- バックエンド：
- AI / API：
- データベース：
- インフラ：
- その他：

## 今後の展望

今後追加したい機能や改善したい点を記載してください。

## セットアップ方法

ローカルで実行する場合の手順を記載してください。

例

```bash
git clone <repository-url>
cd <repository-name>

# 必要なライブラリをインストール
...

# 起動
...
```

## メンバー

| 名前 | 担当 |
|------|------|
|      |      |
|      |      |
